<template>
  <div class="linkage">
    <div>
      <el-select v-model="sheng" @change="choseProvince" placeholder="请选择省份">
        <el-option v-for="item in province" :key="item.id" :label="item.value" :value="item.id"></el-option>
      </el-select>
      <el-select v-model="shi" @change="choseCity" placeholder="请选择城市">
...
const cityOptions = ["北京市", "天津市", "上海市", "安徽省"];
export default {
  data() {
    return {
        ...
        .then(function(response) {
          if (response.status == 200) {
            ...
            for (var item in data) {
              if (item.match(/0000$/)) {
                //省
                for (var cities in that.checkedCities) {
                  if (that.checkedCities[cities] == data[item]) {
                    that.province.push({
                      id: item,
                      value: data[item],
                      children: []
